...
Jii.defineClass('Jii.base.ActiveRecord', {
	...
	__static: {
		 ...
		 *
		 * @param {Jii.sql.BaseActiveRecord} record the record to be populated. In most cases this will be an instance
		 * created by [[instantiate()]] beforehand.
		 * @param {object} row attribute values (name => value)
		 */
		populateRecord: function (record, row) {
			return record.attributes().then(Jii._.bind(function(columns) {
	 ...
	 * @param {Jii.sql.ActiveRecordInterface|[]|null} records the related records to be populated into the relation.
	 */
	...
	save: function (runValidation, attributeNames) {
		runValidation = !Jii._.isUndefined(runValidation) ? runValidation : true;
	...
	updateAttributes: function (attributes) {
		...
		return this.getOldPrimaryKey(true).then(Jii._.bind(function(oldPrimaryKey) {
	...
	_updateInternal: function (attributes) {
		...
		return this.beforeSave(false).then(Jii._.bind(function(bool) {
			...
			return this.getOldPrimaryKey(true).then(Jii._.bind(function(condition) {
	...
	updateCounters: function (counters) {
		...
		}, this)).then(Jii._.bind(function(affectedRows) {
			if (affectedRows === 0) {
				return Jii.when.resolve(false);
			}
	 ...
	 */
	refresh: function () {
		return this.getPrimaryKey(true).then(Jii._.bind(function(primaryKey) {
				return this.findOne(primaryKey);
		}, this)).then(Jii._.bind(function(record) {
			if (record === null) {
				return Jii.when.resolve(false);
			}
	...
	equals: function (record) {
		...
		return Jii.when.all([this.getPrimaryKey(), record.getPrimaryKey()]).then(function(pk1, pk2) {
	...
	getPrimaryKey: function (asArray) {
		...
		return this.primaryKey().then(Jii._.bind(function(keys) {
		...
		return Jii.when.resolve().then(Jii._.bind(function() {
			if (relation.via !== null) {
				...
				if (Jii._.isArray(relation.via)) {
					...
					viaTable = Jii._.first(relation.via.from);
				}
				...
				Jii._.each(extraColumns, Jii._.bind(function(v, k) {
					columns[k] = v;
				}, this));
			...
			return Jii.when.all([
					model.isPrimaryKey(Jii._.keys(relation.link)),
					this.isPrimaryKey(Jii._.values(relation.link))
				]).then(Jii._.bind(function(p1, p2) {
					if (p1 && p2) {
						if (this.isNewRecord() && model.isNewRecord()) {
		...
		return Jii.when.resolve().then(Jii._.bind(function() {
			...
			return Jii.when.all([
				...
				if (Jii._.has(this._related, name)) {
					...
					Jii._.each(this._related[name], Jii._.bind(function(b, a) {
						var promise = Jii.when.all([
							model.getPrimaryKey(),
							b.getPrimaryKey()
						]);
						promises.push(promise);

						promise.then(Jii._.bind(function(pk1, pk2) {
							if (pk1 === pk2) {
