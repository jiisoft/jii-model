...
Jii.defineClass('Jii.base.Model', /** @lends Jii.base.Model.prototype */{
     ...
     * @returns {{}}
     */
    getAttributesTree: function (names) {
    ...
    _buildTree: function(names, model) {
        var obj = {};
        Jii._.each(names, function (child, name) {
            var value = model.get(name);
            if (value instanceof Jii.base.Model) {
                obj[name] = this._buildTree(child, value);
            } else if (value instanceof Jii.base.Collection) {
                obj[name] = Jii._.map(value.getModels(), function(item) {
                    return this._buildTree(child, item);
                }.bind(this));
            } else {
                obj[name] = value;
            }
     ...
     * @returns {string}
     */
    getAttributeHint: function (name) {
