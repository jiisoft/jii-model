...
Jii.defineClass('Jii.data.Model', {
	...
	createValidators: function () {
		...
		_.each(this.rules(), _.bind(function (rule) {
			if (rule instanceof Jii.data.validator.Validator) {
				...
				var validator = Jii.data.validator.Validator.create(rule[1], this, attributes, params);
	...
	isAttributeRequired: function (attribute) {
		...
		_.each(this.getActiveValidators(attribute), _.bind(function(validator) {
			if (validator instanceof Jii.data.validator.RequiredValidator && validator.when === null) {
				bool = true;
			}
		}, this));
		return bool;
